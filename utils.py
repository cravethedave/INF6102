...
class Pizzeria:
    ...
    def generate_cost_and_validity(self, solution: Solution) -> Tuple[float, List[float], bool, List[bool],]:
        ...
        for t, quantity_delivered in solution.pizzeria_pov[self.id].items():
            detailed_quantity_trace.append(
                (level, level + quantity_delivered, level + quantity_delivered - self.dailyConsumption))
            level += quantity_delivered - self.dailyConsumption
            validity_for_each_timestep.append(
                detailed_quantity_trace[-1][-1] >= self.L and detailed_quantity_trace[-1][-2] <= self.U)
...
class Instance:
    ...
    def solution_length_is_valid(self, solution: Solution):
        ...
        if len(solution.raw) != self.T:
            print(
                f"This doesn't look like a correct solution, you must specify {self.T} instead of {len(solution.raw)} timesteps\n")
            return False
        for t, timestep in enumerate(solution.raw):
            if len(timestep) != self.M:
                print(
                    f"This doesn't look like a correct solution, you must specify {self.M} trucks at timestep {t} instead of {len(timestep)} trucks\nYou can specify trucks with empty routes.")
                return False
        return True
    ...
    def solution_route_cost_and_validity(self, solution: Solution) -> Tuple[
        float, List[List[float]], bool, List[List[bool]]]:
        """
        ...
        return sum([sum(x) for x in cost_for_each_timestep_route]), \
            ...
            sum([sum(x) for x in validity_for_each_timestep_route]) == len(validity_for_each_timestep_route) * len(
                validity_for_each_timestep_route[0]), \
    ...
    def solution_cost_and_validity(self, solution: Solution) -> Tuple[float, bool]:
        ...
        return pizzeria_cost + mine_cost + route_cost, (
                pizzeria_validity and route_validity and mine_validity and timestep_validity)
    ...
    def visualize_inventory_sets(self, sol: Solution, show=True):
        ...
        for i, c in enumerate(zip(triples_sets, self.pizzerias)):
            ...
            for j in range(num_bars):
                ax.bar([z + j * bar_width for z in index], [triple[j] for triple in triples], bar_width,
                       label=['Avant la livraison', 'Après la livraison', 'Après la consommation'][j])
            ...
            ax.set_title(f'Inventaire - Pizzeria {i + 1} - $c_{i + 1}={pizzeria.ic}$ - $r_{i + 1}={pizzeria.r}$',
                         color='black' if costandv[i][-3] else 'red')
    ...
    def visualize_mine(self, sol: Solution, show=True):
        ...
        for i, c in enumerate(zip(triples_sets, [self.mine])):
            triples, pizzeria = c
            for j in range(num_bars):
                plt.bar([z + j * bar_width for z in index], [triple[j] for triple in triples], bar_width,
                        label=['Avant la livraison', 'Après la livraison', 'Après la consommation'][j])
    ...
    def generate_setup_visualization(self, sol: Solution, mine_quantity: float, customer_quantities: Dict[int, float],
                                     title='', image_id: int = 0):
        ...
        for mine in mines:
            ...
            t = plt.text(mine.x - 5, mine.y - 15, mine_quantity, verticalalignment='bottom', horizontalalignment='left',
        ...
        for id, customer in customers.items():
            ...
            plt.text(customer.x - 5, customer.y, str(customer.id), verticalalignment='bottom',
                     horizontalalignment='right', fontsize=16)
            q = customer_quantities[customer.id]
            t = plt.text(customer.x - 5, customer.y - 20,
                         str(customer.L) + "$\leq$" + str(q) + "$\leq$" + str(customer.U), verticalalignment='center',
                         horizontalalignment='left', fontsize=10, color='yellow' if q / customer.U > .3 else 'black',
                         fontweight='bold')
            t.set_bbox(dict(facecolor='green' if customer.L <= q and q <= customer.U else 'red',
                            alpha=q / customer.U if customer.L <= q and q <= customer.U else 1,
                            edgecolor='green' if customer.L <= q and q <= customer.U else 'red'))
    ...
    def visualize_route(self, truck_id: int, route: Route, mine_quantity: float, customer_quantities: Dict[int, float],
                        title="", image_id: int = 0):
        customers = self.pizzeria_dict
        mine = self.mine
        ...
        plt.plot(mine.x, mine.y, 'bs', markersize=10, label=f'Mine {mine.id}')
        t = plt.text(mine.x - 5, mine.y - 15, new_depq, verticalalignment='bottom', horizontalalignment='left',
                     fontsize=10, color='yellow', fontweight='bold')
        t.set_bbox(dict(facecolor='blue', alpha=0.5, edgecolor='blue'))
        ...
        for id, customer in customers.items():
            ...
            t = plt.text(customer.x - 5, customer.y - 20,
                         str(customer.L) + "$\leq$" + str(q) + "$\leq$" + str(customer.U), verticalalignment='bottom',
                         horizontalalignment='left', fontsize=10, color='yellow' if q / customer.U > .3 else 'black',
                         fontweight='bold')
            t.set_bbox(dict(facecolor='green', alpha=q / customer.U if customer.L <= q and q <= customer.U else 1,
                            edgecolor='green'))
        ...
        for a, b in zip(list(zip(x_values, y_values))[1:], list(zip(x_values, y_values))[:-1]):
            plt.arrow(b[0], b[1],
                      a[0] - b[0],
                      a[1] - b[1],
                      shape='full', color=arrow_color, length_includes_head=True, head_width=3,
                      head_length=max(25 - i * 3, 10), alpha=.7)
        ...
        plt.title(f'{title}\n Route cost: {route.distance_cost}, truck load: {route.total_goods}/{self.Q}',
                  color='black' if route.total_goods <= self.Q else 'red')
        plt.grid(True)
        os.makedirs(self.visualization_directory, exist_ok=True)
        plt.savefig(os.path.join(self.visualization_directory, f'plot_{image_id}.png'))
        plt.clf()
    ...
    def visualize(self, sol):
        ...
        for t in range(inst.T):
            ...
            inst.generate_setup_visualization(sol, mine_quantity_pre, pizzeria_quantities,
                                              f'Situation at time t={t} (Pre-production)', idx)
            idx += 1
            inst.generate_setup_visualization(sol, mine_quantity_post, pizzeria_quantities,
                                              f'Situation at time t={t} (Post-production)', idx)
            idx += 1
            for truck in range(inst.M):
                ...
                idx += 1
            inst.generate_setup_visualization(sol, mine_quantity_post,
                                              {c.id: c.generate_cost_and_validity(sol)[-1][t][-1] for c in
                                               inst.pizzerias}, f'Situation at time t={t} (Post-consumption)', idx)
            idx += 1
