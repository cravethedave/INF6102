...
if __name__ == '__main__':
    grade = 0
    for instanceId, optimum, factor, maxgrade in zip(['A', 'B', 'C', 'D', 'E'],
                                                     [1640.93, 2475.88, 2061.27, 2911.11, 3487.12], [2, 2, 2, 3, 3],
                                                     [1, 2, 2, 2, 2]):
        try:
            ...
            try:
                ...
                print(f'instance{instanceId} : ',
                      round((maxgrade - maxgrade * (cost - optimum) / (factor * optimum - optimum)) * int(validity),
                            maxgrade), f'/{maxgrade}')
            except FileNotFoundError as e:
                print(f'instance{instanceId} : ', 0,
                      f'/{maxgrade} (file ./solutions/instance{instanceId}.txt not found)')
                grade += 0
        except FileNotFoundError as e:
